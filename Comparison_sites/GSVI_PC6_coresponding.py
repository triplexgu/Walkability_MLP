...
POIs = pd.concat(dfs,axis=0).drop(columns=['GVI'])
combined = pd.merge(POIs,coords[['OBJECTID','X','Y']],left_on='object_id',right_on='OBJECTID').drop(columns='OBJECTID')
# X:lon Y:lat
...
#PYMEANSHIFT
totGVI.to_file(r'D:\WUR\master\MLP\master thesis\data\Comparison_sites\withINFO_totGVI_DeepLab.geojson',drive='GeoJson',encoding='utf-8')
#check how it looks
print(totGVI.head())